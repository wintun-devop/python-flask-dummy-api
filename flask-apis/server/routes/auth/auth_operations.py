    ...
    try:
        ...
        if check_email_exist is not None:
            ...
            if isPasswordCorrect:
                ...
                print("token_attribut",token_attributes)
                access_token = create_access_token(identity=str(check_email_exist.id),additional_claims=token_attributes,fresh=True)
                refresh_token = create_refresh_token(identity=str(check_email_exist.id),additional_claims=token_attributes)
        ...
        if check_id_exist is not None:
            ...
            if isPasswordCorrect:
                ...
                access_token = create_access_token(identity=str(check_id_exist.id),additional_claims=token_attributes,fresh=True)
                refresh_token = create_refresh_token(identity=str(check_id_exist.id),additional_claims=token_attributes)
                response=jsonify({**token_attributes,"access_token": access_token,"refresh_token": refresh_token,"authenticated":True})
                # optional cookies usage
                set_access_cookies(response, access_token)
                set_refresh_cookies(response, refresh_token)
